...
class SList {
  ...
  // 1. removeVal
  removeVal(val) {
    if (this.isEmpty == false && this.head.data == val) {
      this.removeHead();
    }
    // [] - remove the node with the specified value
    // [x] - return the removed val, or null if nothing was removed
    let runner = this.head;
    while (runner) {
      ...
      // reattach the nextnext data person link thing before booping the node
      runner = runner.next;
    }
    return null;
  }
